...
variable "availability_zones" {
  description = "Availability zones for subnet placement (minimum 2 for HA)"
  type        = list(string)
  default     = ["us-east-1a", "us-east-1b"]

  validation {
    condition     = length(var.availability_zones) >= 2
    error_message = "At least 2 availability zones are required for production HA."
  }
}
...
variable "deploy_endpoint" {
  description = "Whether to deploy the SageMaker inference endpoint"
  type        = bool
  default     = false
}
